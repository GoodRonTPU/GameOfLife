...
void MainWindow::saveGame()
{
    ...
    if(!file.open(QIODevice::WriteOnly | QIODevice::Truncate))
        return;
    QString s = QString::number(game->gamefield->cellNumber())+"\n";
    file.write(s.toAscii());
    file.write(game->gamefield->dump().toAscii());
...
void MainWindow::loadGame()
{
    ...
    game->gamefield->setCellNumber(sv);
    QString dump="";
    for(int k=0; k != sv; k++) {
        QString t;
        in >> t;
    ...
    game->gamefield->setDump(dump);

    int r,g,b; // RGB color
    in >> r >> g >> b;
    QColor currentColor = QColor(r,g,b);
...
{
    QColor color = QColorDialog::getColor(QColor("#000"), this, tr("Select color of figures"));
    if(!color.isValid())
        return;
